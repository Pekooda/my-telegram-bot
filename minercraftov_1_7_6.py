...
### ХЛАМ
comasiv = ["start", "guide", "data", "admin", "gm", "id", "like", "nolike", "mathi", "wts", "rs", "rp", "rv", "ttm", "pokapoka", "text", "rr40", "osc", "ors", "ree", "roo"]
...
### ШАНСЫ
    if message.chat.id == OT_ID and random.random() < 0.0003619:
        if random.random() < 0.00001:
            await message.reply("Если вы видите это сообщение, то что-то явно не так со случайностью.") 
        if random.random() < 0.0001:
            await message.reply("Написаное мной за гранью реальности.") 
        if random.random() < 0.001:
            await message.reply("Я существую?") 
