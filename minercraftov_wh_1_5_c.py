...
## Реакция на текст в сообщении
    ...
    if message.animation or message.sticker:
        if message.from_user.id == TIM_ID:
            ...
            chest["timrep"][f"{unique_id}"]["VALUE"] += 1
            if chest["timrep"][f"{unique_id}"]["VALUE"] > maxgif:
                return await bot.delete_message(chat_id=chat_id, message_id=message.message_id)
            chest["timtim"] += "тим "
            await message.answer(chest["timtim"])
            for key in chest["kazn"]:
    ...
    if message.photo and message.from_user.id == TIM_ID:
        ...
        response = requests.post(
            ...
            data={"apikey": "helloworld", "language": "rus"}
        )
        result = response.json()
        testube = result["ParsedResults"][0]["ParsedText"].lower().replace(" ", "")
        if any(word.lower().startswith(mat) for word in testube for mat in MATUUUK):
            chest["NUMB"] = chest["NUMB"] + 2
            await orluk(message, "")
    closechest(chest)
...
async def pivtime():
    while True:
        ...
        for i in range(60):
            ...
            for key in chest["kazn"]:
                if chest["kazn"][key]["TYPE"] == "time" and chest["kazn"][key]["NOW"] > 0:
                    chest["kazn"][key]["NOW"] -= 1
            for key in chest["timrep"]:
                if chest["timrep"][key]["TIME"] > 0:
                    chest["timrep"][key]["TIME"] -= 1
                if chest["timrep"][key]["TIME"] == 0 and chest["timrep"][key]["VALUE"] > 0:
                    chest["timrep"][key]["VALUE"] -= 1
                    if chest["timrep"][key]["VALUE"] > 0:
                        chest["timrep"][key]["TIME"] == timeout
            closechest(chest)
        if chest["NUMB"] > 0:
            chest["NUMB"] -= 1
            closechest(chest)
