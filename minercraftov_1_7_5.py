...
# 1.7 - осц стикерпаки
# 1.7.1 - тим попросил
# 1.7.2 - больше матюков
# 1.7.3 - /re
# 1.7.4 - тим попросил мут по кнопке. orluk теперь не вызывается.
# 1.7.5 - цитирую.
...
from aiogram.types import Message, Update, ChatPermissions, FSInputFile, BufferedInputFile, MessageEntity
...
NO_MATUK = json.loads(os.getenv("E_NO_MATUK", "[]"))
...
### РП/В с текстом
async def orluk(message: types.Message, args: str, cap: str):
    tes = random.choice([rv, rp])
    eagle = await tes(message, args)
    if tes == rp:
        await message.answer_photo(eagle[1], caption=cap)
    else:
        await message.answer_animation(eagle[1], caption=cap)
...
async def roo(message: types.Message, args: str):
    ...
    if chat_id != OT_ID and chat_id != OTOLD_ID:
        return
    numba = random.randint(1, 1026382)
...
## Реакция на полный текст
    if message.text:
        ...
        anada = re.search(r'2763', message.text, re.IGNORECASE)
        if anada:
            FORESTS = message.text[anada.start():anada.end()]
            await message.answer_sticker(sticker=LEAFY_NIQ, reply_parameters={"message_id": message_id, "quote": FORESTS})
## Реакция на текст в сообщении
        ans = re.search(r'все', message.text, re.IGNORECASE)
        if ans and message.chat.id == OT_ID and random.random() < 0.01:
            FORES = message.text[ans.start():ans.end()]
            await message.answer_sticker(sticker=VSE_NIQ, reply_parameters={"message_id": message_id, "quote": FORES})
        anad = re.search(r'майнера крафтов', message.text, re.IGNORECASE)
        if anad:
            FORESTA = message.text[anad.start():anad.end()]
            await message.answer("не зли меня, бяка >=(", reply_parameters={"message_id": message_id, "quote": FORESTA})
        if message.from_user.id == TIM_ID:
            ana = re.search(r'\bдак\b', message.text, re.IGNORECASE)
            if ana:
                FOREST = message.text[ana.start():ana.end()]
                await message.answer("🦆", reply_parameters={"message_id": message_id, "quote": FOREST})
## Реакция на текст в сообщении
        if (message.from_user.id == TIM_ID and (any(word.lower().startswith(mat) for word in message.text.split() for mat in MATUUUK) or any(matu in message.text.lower() for matu in FUL_MATUUUK))) and not any(mata in message.text.lower() for mata in NO_MATUK):
            ...
            await orluk(message, "орел", "ОРЛЮКИ ОБКЛЕВАЛИ СООБЩЕНИЕ ТИМА И ЕГО ЖЕ САМОГО НА 5 МИНУТ!")
            await bot.restrict_chat_member(chat_id=OT_ID, user_id=TIM_ID, permissions=ChatPermissions(can_send_photos=False))
    if message.photo and message.from_user.id == TIM_ID:
        try:
            ...
            if (any(word.lower().startswith(mat) for word in testube.split() for mat in MATUUUK) or any(matu in areyousure for matu in FUL_MATUUUK)) and not any(mata in areyousure for mata in NO_MATUK):
                try:
                    matmat = True
                    await orluk(message, "орел", "ты не думай, что я не вижу матюки в картинках, тим")
                    await bot.delete_message(chat_id=chat_id, message_id=message_id)
                    await bot.restrict_chat_member(chat_id=OT_ID, user_id=TIM_ID, permissions=ChatPermissions(can_send_photos=False))
                except Exception as e:
                    await bot.send_message(PEKO_ID, f"??!? {e}")
        except Exception as e:
            pass
    closechest(chest)
